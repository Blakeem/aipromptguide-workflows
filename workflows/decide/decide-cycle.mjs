export const meta = {
  ...
  phases: [
    ...
    { title: 'Decide', detail: 'The decider reads the requirements + every lens file (+ the latest review), combines the best elements, builds a GLOBAL weighted decision matrix enforcing the non-negotiables, picks the winner, and writes decision-rN.md. Returns the choice + a meets-all-requirements flag.' },
    { title: 'Review', detail: 'A NON-BLIND adversarial reviewer reads the decision + requirements + lens files, checks every requirement is met and the matrix is sound (no unsupported leap, no better option overlooked), and writes decision-review-rN.md. Agreement ends the loop; gaps drive another Decide round.' },
  ],
};
...
if (!LENSES.length) {
  throw new Error('args.lenses is required: a non-empty array of evaluation perspectives (strings, or { id, focus }), e.g. ["efficiency","simplest","robustness","best-practice"]. The main agent picks these with the user beforehand (#4).');
}
...
const ANALYST_SCHEMA = {
  ...
  properties: {
    ...
    options_count: { type: 'integer', description: 'how many distinct options you generated (aim 2–4)' },
...
const DECIDE_SCHEMA = {
  type: 'object',
  required: ['wrote_file', 'chosen', 'meets_all_requirements', 'needs_user'],
  properties: {
...
Return wrote_file + options_count + top_pick via the schema (the analysis itself is the file).`;
...
${LENSES.map((l) => `  - ${lensFile(l.id)}  (lens: ${l.focus})`).join('\n')}
${round === 1
    ? `This is round 1.`
...
// anchor it) — it re-checks the CURRENT decision fresh against the fixed rubric.
...
THE LENS ANALYSES it drew on (cross-check its claims against these): ${LENSES.map((l) => lensFile(l.id)).join(', ')}
...
2. Every weighted criterion actually addressed, and the scores justified by the lens evidence — not
   asserted? Re-derive any score that looks generous.
...
return {
  ...
  lensFiles: LENSES.map((l) => lensFile(l.id)),
