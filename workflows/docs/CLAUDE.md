...
and checks **cross-source consistency + coverage**; gaps it finds (missing coverage *or* files needing
recapture) drive a bounded gap-fill gather round. Built to `../../principles/WORKFLOW-PRINCIPLES.md` — a
...
**no claim-verifier by design**:
a verifier exists to catch the gap between a gatherer's *paraphrase* and its source, and verbatim capture
leaves no such gap. What remains checkable — coverage and cross-source consistency — needs the whole set
at once, so it lives in the curator (#4). Nothing is written outside `outDir`, staged, or committed.
...
1. **Frame the brief + sources + destination with the user.** The brief says what the docs are **for** —
   ...
   live** (`outDir`): usually the project's docs folder with a subfolder per system/API — e.g.
   `<repo>/docs/stripe/` — created if missing; no destination → run-state, copy out later. A complex
   brief goes in **`EnterPlanMode`**, approve, pass `planPath`; a simple one inline as `brief`.
...
3. **Present** the set: read `INDEX.md` (including **Coverage notes**) and relay what was gathered, any
   cross-source inconsistencies, and unresolved gaps. The set lives in gitignored run-state — copy it
   into the project (or re-run with `outDir`) if it should persist. Point the working agent/plan at the
   INDEX.
...
- **`outDir` (optional but usually wanted):** where the curated set lands — decide it with the user
  (§2.1): usually `<project>/docs/<system-or-api>/`, created if missing; still never staged or
  committed. Default without it: `runs/<runId>/docs` (gitignored run-state you copy out later).
...
Each source is one gatherer: `{ id, kind, focus }` (a bare string = a web source).
...
- **Curator** (curate · sonnet) — one per round, after all sources land (the one genuine barrier —
  ...
  (one line per file + Coverage notes), and returns gaps a fresh gather could fix — missing coverage
  **or** a recapture (wrong version pulled, paraphrased file, missing header).
...
- **Bounded gap loop.** `[gather → scrub → curate]×N`, up to `maxRounds` (default 2). Gaps returned are
  ...
  version, paraphrase); their rationale lives in Coverage notes. Gaps still open at the bound stay
  listed there for the user.
...
- **Read-only outside `outDir`.** The engine writes only the doc set + index; it never touches, stages,
  or commits any repo.
...
- **Optional:** `outDir` (usually `<project>/docs/<system>/`; default `runs/<runId>/docs`) · `maxRounds`
...
Report when done: the folder + `INDEX.md` paths, file count, rounds run, inconsistencies, and unresolved
gaps — then relay the Coverage notes. **Nothing is staged or committed**; copy the set into the project
(or re-run with `outDir`) if it should persist.
