...
// pre-existing work as this section's and acceptance stages it into the accepted baseline.
{
  for (const bad of [null, false, '', []]) {
    const { logs } = await run({ ...GREEN_RUN, 'develop': { ...DEV, baseline_dirty_files: bad } });
...
// prescribes routed to case 6 (DROP — LOG), the reviewer re-raised it, and the round budget burned.
{
  ...
  ok(/DROP \(1 or 6b\): append ONE terse line to E:\/r\/runs\/t\/gate\/DISMISSED-sec-a\.md/.test(dev),
 ...
 * enumerates the section's acceptance criteria, `AMENDED-<id>.md` quotes the overridden section clause
 * verbatim, and NEEDS-USER.md carries the pointer line naming that file — and #3 forbids relying on
 * "please don't read X". So the reviewer's own two files live one directory down, and this is the
 * assertion that keeps a later edit from interpolating a state-dir path back into the prompt.
 */
...
// critic reads the section clause it quotes verbatim, and the unbiased first stage this engine is built on
// is gone.
{
  const q = (await run(GREEN_RUN)).prompt('quality sec-a');
  ok(q !== '', 'the blind reviewer ran');
  ok(q.includes(`${STATE}/gate/DISMISSED-sec-a.md`),
    'it IS given the anti-spin ledger — fresh, but not amnesiac (#5)');
  ok(q.includes(`${STATE}/gate/quality-review-sec-a-r1.md`), 'and its own output path, gate-scoped too');
  ok(!q.includes('NEEDS-USER.md'),
    'but NOT NEEDS-USER: its amendment pointer line names the AMENDED file, which quotes the section verbatim');
...
// changes, and the pair spins to maxRounds: a section that used to accept PARKS, and here a park STOPS the
// run. The anti-spin channel (#5) has to reach the reviewer through the ONE file it is still handed.
{
  const dev = (await run(GREEN_RUN)).prompt('develop sec-a');
  ok(/ESCALATE \(7\)[\s\S]*?append ONE terse line to E:\/r\/runs\/t\/gate\/DISMISSED-sec-a\.md/.test(dev),
...
section('the AMENDED record never reaches the BLIND reviewer, and SETTLED hands it the gate ledger alone');
{
  ...
  ok(/E:\/r\/runs\/t\/gate\/DISMISSED-sec-a\.md/.test(q) && !/NEEDS-USER/.test(q),
    'SETTLED hands it the gate-scoped DISMISSED ledger and nothing else');
  const a = (await run(GREEN_RUN)).prompt('acceptance sec-a');
  ok(/E:\/r\/runs\/t\/gate\/DISMISSED-sec-a\.md/.test(a) && /E:\/r\/runs\/t\/NEEDS-USER\.md/.test(a),
